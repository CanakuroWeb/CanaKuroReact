...
function Todo_board() {
  ...
  const getOnlineData = async () => {
    await axios
      .get("https://raw.githubusercontent.com/CanakuroWeb/CanakuroWeb.github.io/dev/test2/src/sample_data/todo_sample.json")
      .then(response => {
        ...
        {todoList.todoList.map(todo => {
          ...
          return (
            <div
              key={todo.id}
              className={
                "todo__item" + (todo.killing ? " todo__remove" : "") // 킬되는 순간.... 지연을 받고 삭제
              }
            >
              <div className="todo__text">
                <span style={todo.doneMark ? { textDecorationLine: "line-through" } : {}}>
